...
class MLKeyloggerDetector:
    ...
    def __init__(self, model_path: str = "models/development", 
    ...
    def _load_sklearn_model(self):
        ...
        try:
            model_path = self.model_path / "modelo_keylogger_from_datos.pkl"
            if not model_path.exists():
                raise FileNotFoundError(f"Modelo sklearn no encontrado: {model_path}")
    ...
    def extract_features_from_network_data(self, network_data: List[Dict]) -> np.ndarray:
        ...
        try:
            ...
            # Convertir datos de red a DataFrame
            df = pd.DataFrame(network_data)
            
            # Crear características agregadas por flujo de red
            features_list = []
            
            # Agrupar por conexión (src_ip, dst_ip, src_port, dst_port)
            if all(col in df.columns for col in ['src_ip', 'dst_ip', 'src_port', 'dst_port']):
                grouped = df.groupby(['src_ip', 'dst_ip', 'src_port', 'dst_port'])
                
                for (src_ip, dst_ip, src_port, dst_port), group in grouped:
                    flow_features = self._extract_flow_features(group)
                    features_list.append(flow_features)
            else:
                # Si no tenemos datos de red completos, usar datos agregados
    ...
    def _extract_flow_features(self, flow_data: pd.DataFrame) -> List[float]:
        """Extrae características de un flujo de red específico"""
        features = []
        
        try:
            # Características básicas del flujo
            flow_duration = flow_data['timestamp'].max() - flow_data['timestamp'].min() if 'timestamp' in flow_data.columns else 0
            total_packets = len(flow_data)
            
            # Características de paquetes
            if 'packet_size' in flow_data.columns:
                packet_sizes = flow_data['packet_size']
                total_bytes = packet_sizes.sum()
                avg_packet_size = packet_sizes.mean()
                packet_size_std = packet_sizes.std()
            else:
                total_bytes = avg_packet_size = packet_size_std = 0
            
            # Características direccionales (forward/backward)
            fwd_packets = len(flow_data[flow_data.get('direction', 'fwd') == 'fwd']) if 'direction' in flow_data.columns else total_packets // 2
            bwd_packets = total_packets - fwd_packets
            
            # Construir vector de características (simplificado)
            # En un sistema real, esto sería más complejo y específico al dataset
            features = [
                flow_duration,                    # Flow Duration
                fwd_packets,                      # Total Fwd Packets
                bwd_packets,                      # Total Backward Packets
                total_bytes * 0.6,               # Total Length of Fwd Packets (estimado)
                total_bytes * 0.4,               # Total Length of Bwd Packets (estimado)
                avg_packet_size * 1.2,           # Fwd Packet Length Max (estimado)
                avg_packet_size * 0.8,           # Fwd Packet Length Min (estimado)
                avg_packet_size,                  # Fwd Packet Length Mean
                packet_size_std,                  # Fwd Packet Length Std
                avg_packet_size * 1.1,           # Bwd Packet Length Max (estimado)
                avg_packet_size * 0.9,           # Bwd Packet Length Min (estimado)
                avg_packet_size,                  # Bwd Packet Length Mean
                packet_size_std,                  # Bwd Packet Length Std
                total_bytes / max(flow_duration, 1),  # Flow Bytes/s
                total_packets / max(flow_duration, 1), # Flow Packets/s
            ]
            
            # Rellenar con valores por defecto para el resto de características
            ...
            logger.error(f"[ERROR] Error en _extract_flow_features: {e}")
