...
int main(int argc, char **argv) {
    ...
    if (!fs::exists(log_file.parent_path())) fs::create_directories(log_file.parent_path());

    std::ofstream log(log_file, std::ios::app);
    if (!log) {
        std::cerr << "No se pudo abrir el archivo de log: " << log_file << std::endl;
        return 1;
    }
    ...
    while (keep_running) {
        ...
        if (enabled) {
            ...
            cmd << "python scripts/predecir_keylogger.py --onnx \"" << model << "\" --input \"" << input << "\" >> \"" << log_file << "\" 2>&1";
