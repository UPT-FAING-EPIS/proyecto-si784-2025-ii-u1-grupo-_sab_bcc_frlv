...
Uso:
  python convert_pkl_to_onnx.py --pkl modelos/modelo_keylogger_from_datos.pkl --features modelos/modelo_keylogger_from_datos_features.json --out modelos/modelo_keylogger_from_datos.onnx
...
DEFAULT_PKL = Path('modelo_keylogger_from_datos.pkl')
DEFAULT_FEATURES = Path('modelo_keylogger_from_datos_features.json')
DEFAULT_OUT = Path('modelo_keylogger_from_datos.onnx')
...
def main():
    ...
    if not args.pkl.exists():
        print(f"Error: no se encontró el .pkl en {args.pkl.resolve()}")
        sys.exit(1)
    if not args.features.exists():
        print(f"Error: no se encontró el features.json en {args.features.resolve()}")
        sys.exit(2)
    ...
    try:
        ...
        # Guardar en la carpeta actual (modelo_datasets)
        with open(args.out, 'wb') as fh:
            fh.write(onnx_model.SerializeToString())
        print(f"Guardado ONNX en {args.out.resolve()}")
    except Exception as e:
