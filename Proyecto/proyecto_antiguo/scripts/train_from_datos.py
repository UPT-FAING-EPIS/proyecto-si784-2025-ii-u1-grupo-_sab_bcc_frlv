...
def gather_label_values(files, label_col, sample_rows=1000):
    vals = set()
    for f in files:
        try:
            for chunk in pd.read_csv(f, usecols=[label_col], chunksize=sample_rows, low_memory=False):
                vals.update(chunk[label_col].dropna().unique().tolist())
                if len(vals) > 1000:
                    break
        except Exception:
            continue
...
def iter_numeric_chunks(files, label_col, numeric_cols, max_rows=None, chunksize=100000, sample_frac=None):
    collected = 0
    parts = []
    problematic_examples = {c: set() for c in numeric_cols}
    for f in files:
        try:
            for chunk in pd.read_csv(f, usecols=numeric_cols + [label_col], chunksize=chunksize, low_memory=False):
                chunk.columns = [c.strip() for c in chunk.columns]
                chunk = chunk.fillna(0)
                # detect problematic non-numeric values while coercing
                for c in numeric_cols:
                    coerced = pd.to_numeric(chunk[c], errors="coerce")
                    # examples where coercion produced NaN but original was not empty
    ...
    for f in files:
        try:
            ...
            if label_col not in sample.columns:
                continue
            numeric = [c for c in sample.columns if c != label_col and pd.api.types.is_numeric_dtype(sample[c])]
            if numeric:
                return numeric
        except Exception:
            continue
...
def main():
    ...
    # separate X/y
    X = df[numeric_cols].astype(np.float32).fillna(0)
    y_raw = df[label_col].fillna("")
    ...
    if args.onnx:
        try:
            ...
            initial_type = [("float_input", FloatTensorType([None, len(numeric_cols)]))]
