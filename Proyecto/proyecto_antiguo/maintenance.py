...
class AntivirusMaintenance:
    ...
    def run_maintenance(self, mode='standard'):
        ...
        print("🧹 SISTEMA DE MANTENIMIENTO DEL ANTI-KEYLOGGER")
        print("=" * 55)
        ...
        for log_file in logs_dir.glob("*.log"):
            try:
                ...
                if file_time < cutoff_date:
                    ...
                    print(f"   🗑️ Eliminado: {log_file.name} ({file_size_mb:.1f}MB)")
            except Exception as e:
        ...
        for log_file in logs_dir.glob("*.log"):
            try:
                if log_file.stat().st_size > 100 * 1024 * 1024:  # 100MB
                    ...
                    print(f"   🔄 Rotado: {log_file.name} -> {backup_name}")
            except Exception as e:
                print(f"   ⚠️ Error rotando {log_file.name}: {e}")
                self.stats['errors_found'] += 1
        ...
        for temp_file in temp_dir.iterdir():
            try:
                if temp_file.is_file():
                    ...
                    self.stats['files_cleaned'] += 1
                    self.stats['space_freed_mb'] += file_size_mb
                    print(f"   🗑️ Eliminado: {temp_file.name} ({file_size_mb:.1f}MB)")
                elif temp_file.is_dir():
                    shutil.rmtree(temp_file)
                    print(f"   📁 Directorio eliminado: {temp_file.name}")
            except Exception as e:
                print(f"   ⚠️ Error procesando {temp_file.name}: {e}")
                self.stats['errors_found'] += 1
    
    def _clean_quarantine(self):
        """Limpia archivos en cuarentena antiguos"""
        print("🔒 Limpiando cuarentena...")
            ...
            try:
                ...
                if file_time < cutoff_date:
                    if quarantine_file.is_file():
                        ...
                        self.stats['files_cleaned'] += 1
                        self.stats['space_freed_mb'] += file_size_mb
                        print(f"   🗑️ Eliminado de cuarentena: {quarantine_file.name} ({file_size_mb:.1f}MB)")
                    elif quarantine_file.is_dir():
                        shutil.rmtree(quarantine_file)
                        print(f"   📁 Directorio de cuarentena eliminado: {quarantine_file.name}")
            except Exception as e:
                print(f"   ⚠️ Error procesando {quarantine_file.name}: {e}")
                self.stats['errors_found'] += 1
    
    def _clean_cache_files(self):
        """Limpia archivos de caché"""
        print("💾 Limpiando archivos de caché...")
        
        # Limpiar __pycache__
        for pycache_dir in self.root_dir.rglob("__pycache__"):
            try:
                shutil.rmtree(pycache_dir)
                print(f"   🗑️ Eliminado: {pycache_dir.relative_to(self.root_dir)}")
                self.stats['files_cleaned'] += 1
            except Exception as e:
        ...
        for pyc_file in self.root_dir.rglob("*.pyc"):
            try:
                ...
                self.stats['files_cleaned'] += 1
                self.stats['space_freed_mb'] += file_size_mb
                print(f"   🗑️ Eliminado: {pyc_file.relative_to(self.root_dir)}")
            except Exception as e:
        ...
        for pattern in temp_patterns:
            for temp_file in models_dir.glob(pattern):
                try:
                    ...
                    print(f"   ⚠️ Error eliminando {temp_file.name}: {e}")
                    self.stats['errors_found'] += 1
        ...
        for file_path in important_files:
            ...
            if source.exists():
                try:
                    ...
                    print(f"   💾 Backup: {file_path} -> {backup_name}")
                except Exception as e:
                    print(f"   ⚠️ Error en backup de {file_path}: {e}")
                    self.stats['errors_found'] += 1
            else:
        ...
        for pattern in db_patterns:
            for db_file in self.root_dir.rglob(pattern):
                ...
                try:
                    ...
                    print(f"   🔧 Encontrada DB: {db_file.relative_to(self.root_dir)}")
                except Exception as e:
                    print(f"   ⚠️ Error optimizando {db_file}: {e}")
                    self.stats['errors_found'] += 1
    ...
    def _check_disk_space(self):
        ...
        try:
            ...
            if free_gb < 1.0:  # Menos de 1GB libre
                ...
                print("   ⚠️ ADVERTENCIA: Disco casi lleno")
            else:
                print("   ✅ Espacio en disco adecuado")
                
        except Exception as e:
            print(f"   ⚠️ Error verificando espacio: {e}")
            self.stats['errors_found'] += 1
    ...
    def _show_maintenance_summary(self):
        ...
        if self.stats['errors_found'] == 0:
            print("\n✅ MANTENIMIENTO COMPLETADO SIN ERRORES")
        else:
            print(f"\n⚠️ MANTENIMIENTO COMPLETADO CON {self.stats['errors_found']} ERRORES")
